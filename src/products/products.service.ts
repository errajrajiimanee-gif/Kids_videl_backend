...
export class ProductsService {
  private store = new JsonStore<Product>('products.json', [
    {
      id: 1,
  ...
  async findAll(filters?: {
    ...
    if (filters?.brandId) {
      ...
      if (!Number.isNaN(id)) result = result.filter(p => p.brandId === id);
    }
    if (filters?.minPrice) {
  ...
  async create(product: Omit<Product, 'id'>): Promise<Product> {
    return await this.store.create(product as Omit<Product, 'id'>);
  }
