...
exports.buildNotifyPreview = buildNotifyPreview;
...
const normalizeWhatsApp = (value) => {
    ...
    if (raw.startsWith('0') && raw.length === 10)
        return `+212${raw.slice(1)}`;
    if (raw.startsWith('212'))
        return `+${raw}`;
    return raw.startsWith('+') ? raw : `+${raw}`;
};
const buildWhatsAppLink = (phoneE164, text) => {
    const digits = phoneE164.replace(/[^\d]/g, '');
    return `https://wa.me/${digits}?text=${encodeURIComponent(text)}`;
};
const buildMailTo = (email, subject, body) => `mailto:${encodeURIComponent(email)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
const formatMoney = (value) => `${value.toFixed(2)} MAD`;
...
const buildOrderHtml = (order) => {
    ...
    const itemsRows = order.items
      ...
      `.trim();
        ...
        .join('');
    const whatsappToCustomer = customerPhone ? `https://wa.me/${customerPhone.replace(/[^\d]/g, '')}` : '';
    return `
...
function buildNotifyPreview(order) {
    ...
    const whatsappsRaw = parseList(process.env.ADMIN_WHATSAPP_NUMBERS).length > 0
...
async function notifyAdmin(order) {
    ...
    if (twilioSid && twilioToken && twilioFrom) {
        const results = await Promise.all(preview.adminWhatsApps.map(async (phoneE164) => {
...
//# sourceMappingURL=orders.notifications.js.map